...
test_select(User, Pass) ->
    ...
    {ok, _BindVar} = erloci_nif_drv:ociBindByName(Stmthp, <<"A">>, 0, erloci_nif_drv:sql_type('SQLT_CHR'), <<"X">>),
    {ok, _BindVar2} = erloci_nif_drv:ociBindByName(Stmthp, <<"B">>, 0, erloci_nif_drv:sql_type('SQLT_CHR'), <<"Y">>),
    ok = erloci_nif_drv:ociStmtExecute(Svchp, Stmthp, 0, 0, erloci_nif_drv:oci_mode('OCI_DEFAULT')),
    {ok, _Rows} = erloci_nif_drv:ociStmtFetch(Stmthp, 1).
