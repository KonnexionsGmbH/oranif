...
static ERL_NIF_TERM ociEnvNlsCreate(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    case OCI_ERROR:
        ...
        {
            OCIErrorGet(
                envhp, 1, NULL, &errcode, errbuf, sizeof(errbuf),
                OCI_HTYPE_ERROR);
            return RAISE_OCI_EXCEPTION(errcode, errbuf);
        }
        else
        {
            return RAISE_EXCEPTION("NULL Handle");
        }
    case OCI_INVALID_HANDLE:
        return RAISE_EXCEPTION("Invalid Handle");
...
static ERL_NIF_TERM ociCharsetAttrGet(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    status = OCIAttrGet(envhp_res->envhp, OCI_HTYPE_ENV, &charset, NULL,
    ...
    status = OCIAttrGet(envhp_res->envhp, OCI_HTYPE_ENV, &ncharset, NULL,
...
static ERL_NIF_TERM ociAuthHandleCreate(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    ERL_NIF_TERM nif_res = enif_make_resource(env, res);
    enif_release_resource(res);
    return enif_make_tuple(env, 2, ATOM_OK, nif_res);
}
...
static ERL_NIF_TERM ociStmtExecute(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if (stmthp_res->stmt_type == OCI_STMT_INSERT || stmthp_res->stmt_type == OCI_STMT_UPDATE || stmthp_res->stmt_type == OCI_STMT_DELETE)
    {
...
{
    TRACE;
    envhp_resource_type = enif_open_resource_type(
        env, NULL, "envhp", envhp_res_dtor, ERL_NIF_RT_CREATE, NULL);

    spoolhp_resource_type = enif_open_resource_type(
        env, NULL, "spoolhp", spoolhp_res_dtor, ERL_NIF_RT_CREATE, NULL);

    authhp_resource_type = enif_open_resource_type(
        env, NULL, "authhp", authhp_res_dtor, ERL_NIF_RT_CREATE, NULL);

    svchp_resource_type = enif_open_resource_type(
        env, NULL, "svchp", svchp_res_dtor, ERL_NIF_RT_CREATE, NULL);

    stmthp_resource_type = enif_open_resource_type(
        env, NULL, "stmthp", stmthp_res_dtor, ERL_NIF_RT_CREATE, NULL);

    bindhp_resource_type = enif_open_resource_type(
        env, NULL, "bindhp", bindhp_res_dtor, ERL_NIF_RT_CREATE, NULL);
