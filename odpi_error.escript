...
-define(TEST_SQL, <<"select * from foo a, foo b, foo c">>). %selects 1000 rows
% in the DDERL snippet, the variable has size of only 999, that's why it crashes
% in the real DDERL, it only has size 100
% if the size was 1000 or more, the crash doesn't happen with this query
    ...
    VarsDatas = [
            begin
                case NativeType of 'DPI_NATIVE_TYPE_DOUBLE' ->  % if the type is a double, make a variable for it, but say that the native type is bytes
                        ...
                        #{var := Var, data := Datas} = dpi:conn_newVar(Conn, OraType, 'DPI_NATIVE_TYPE_BYTES', 999, 0, false, false, null),
                    ...
                    'DPI_NATIVE_TYPE_LOB' ->
                        #{var := Var, data := Datas} = dpi:conn_newVar(Conn, OraType, 'DPI_NATIVE_TYPE_LOB', 999, 0, false, false, null),
                        ok = dpi:stmt_define(Stmt, Col, Var),    %% results will be fetched to the vars and go into the data
                        {Var, Datas, OraType}; % put the variable and its data list into a tuple
...
main(_) ->
    ...
    % with a block size of 999, it runs, but only fetches 100 results
    % with a block size of 1000 it crashes because then the array is too small
    R = dpi_fetch_rows( Conn, Stmt, 999),
    io:format("Fetched ~p~n", [R]),
    halt(1).
